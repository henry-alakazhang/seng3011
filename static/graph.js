		...
		$.get(
		    ...
		    function(data) {
		    	...
		    	for (var i = 0; i < keys.length; i++) {
			    	...
			    	for (var key in data[keys[i]]) {
			    	  if (data[keys[i]].hasOwnProperty(key)) {
			    	    ric_keys.push(key);
			    	    if ($.inArray(key,rics) == -1) {
			    	    	rics.push(key);
			    	    }
			    	  }			    	  
			    	}
			    	for (var j = 0; j < ric_keys.length; j++) {			    		
				    	for (var key in data[keys[i]][ric_keys[j]]) {
				    	  if (data[keys[i]][ric_keys[j]].hasOwnProperty(key)) {
				    		  if (data[keys[i]][ric_keys[j]][key] > 0) {
	    	   ...
	    	   items = [];
	    		   ...
	    		   items.push('<tr><td><div class="checkbox"><label><input type="checkbox" value="'+ val +'"></label></div></td><td>'+ val +'</td></tr>');
    	   		});
