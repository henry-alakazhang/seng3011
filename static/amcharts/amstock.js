(function() {
    ...
    d.AmStockChart = d.Class({
        ...
        updateData: function() {
            ...
            if (b) {
                a.parsingData = !1;
                var c = a.categoryAxesSettings; - 1 == d.getItemIndex(c.minPeriod, c.groupToPeriods) && c.groupToPeriods.unshift(c.minPeriod);
                ...
                for (l = 0; l < w; l++) {
                    ...
                    for (C = f; C < n; C++) {
                        ...
                        if (C == f || D >= y[t] || !y[t]) {
                            ...
                            if (C == f)
                                for (var I in B) B.hasOwnProperty(I) && (r[t][I] = B[I]);
                            r[t][p] = d.newDate(E, b)
                        } else
                            for (F = 0; F < A; F++) u = z[F].toField, v = r[t], l == w - 1 && (v[p] = d.newDate(E, b)), x = Number(q[u]), isNaN(x) || (isNaN(v[u + "Low"]) && (v[u + "Low"] = x), x < v[u + "Low"] && (v[u + "Low"] = x), isNaN(v[u + "High"]) && (v[u + "High"] = x), x > v[u + "High"] && (v[u + "High"] = x), isNaN(v[u + "AbsHigh"]) && (v[u + "AbsHigh"] = x), Math.abs(x) > v[u + "AbsHigh"] && (v[u + "AbsHigh"] = x), v[u + "Close"] = x, H = d.getDecimals(v[u + "Sum"]), G = d.getDecimals(x),
        ...
        parseEvents: function(a, b,
            ...
            for (p = 0; p < g; p++) {
                ...
                for (f = 0; f < e.length; f++) n = e[f], l = n.graph, d.isString(l) && (l = d.getObjById(w, l)) && (n.graph = l), l && (n.panel = l.chart)
            }
            for (var y in m)
                if (m.hasOwnProperty(y))
                    for (b = m[y], g = d.extractPeriod(y), p = b.length, f = 0; f < e.length; f++) {
                        n = e[f];
                        l = n.date instanceof
                        Date;
            ...
            if (this.div) {
                ...
                if (c != this.prevPH) {
                    ...
                    0 < c && (this.panelsContainer.style.height =
                        c + "px");
                    for (var e = 0, k, b = 0; b < a.length; b++)
                        if (k = a[b]) {
                    ...
                    for (b = 0; b < a.length; b++)
                        if (k = a[b]) k.percentHeight = k.percentHeight / e * 100, k.panelBox && (k.panelBox.style.height = Math.round(k.percentHeight * this.panelsHeight / 100) + "px");
                    this.prevPH = c
                }
            }
        },
        ...
        zoomChart: function(a, b) {
            ...
            if (h && k) {
                ...
                for (h = 0; h < l.length; h++) {
                    ...
                    0 < f.startDuration && c.animationPlayed &&
                        !e ? (k.startDuration = 0, k.animateAgain()) : 0 < f.startDuration && !e && k.animateAgain()
                }
                c.animationPlayed = !0;
                g = d.newDate(b);
                ...
                c.skipEvents || a.getTime() == c.previousStartDate.getTime() && b.getTime() == c.previousEndDate.getTime() || (n = {
                        type: "zoomed"
                    }, n.startDate = a, n.endDate = b, n.chart = c, n.period = m, c.fire(n), c.previousStartDate = d.newDate(a),
            ...
            if (0 < this.dataSets.length) {
                ...
                for (b = 0; b < a.length; b++) {
                    ...
                    for (m = 0; m < k.length; m++) {
                        ...
                        f.useDataSetColors && (f.lineColor = e.color, f.fillColors =
                            void 0, f.bulletColor = void 0);
                        var n = !1,
                            l = e.fieldMappings;
        ...
        markButtonAsSelected: function() {
            if (!this.skipMark) {
                ...
                for (k = b.length - 1; 0 <= k; k--) {
                    var m = b[k],
                        g = m.button;
                    m.startTime && m.endTime && c == m.startTime && e == m.endTime && (this.unselectButtons(), g.className = "amChartsButtonSelected " + a.classNamePrefix + "-period-input-selected",
                        h && d.applyStyles(g.style, h.PeriodButtonSelected))
                }
            }
            this.skipMark = !1
        },
    ...
    d.DataSetSelector = d.Class({
        ...
        write: function(a) {
            ...
            if (0 < b.countDataSets("showInCompare") && 1 < k.length)
                if (m ? (d.addBr(a), d.addBr(a)) : (c = document.createTextNode(" "), a.appendChild(c)), c = document.createTextNode(d.lang.compareText || b.compareText), a.appendChild(c), f = b.listCheckBoxSize, m) {
                    d.addBr(a);
                    ...
                    for (c = 0; c < k.length; c++)
                        if (p = k[c], !0 === p.showInCompare && p != b.chart.mainDataSet) {
                            f = document.createElement("option");
                            f.text = p.title;
                            f.value = c;
